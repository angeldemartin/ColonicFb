...
seuratM$clusterName[which(seuratM$RNA_snn_res.0.25 %in% "10" )] <- "EpC"
...
###order
seuratM$clusterName <- factor(seuratM$clusterName, levels=c("PdgfraloFb1","PdgfraloFb2","Trophocytes1","Trophocytes2","Telocytes/Fibro2","PdgfrahiFb","Myocytes","Glial","ICC1", "ICC2","Immun1","Immun2","EpC","Cycling"))
...
##cond
Idents(seuratM) <- seuratM$cond
DimPlot(seuratM, reduction = "umap", pt.size = 0.2, cols = colcond) 
DimPlot(seuratM, reduction = "umap", pt.size = 0.2, cols = colcond) + theme(legend.position = "none")
...
##RNA_snn_res.0.25.
Idents(seuratM) <- seuratM$RNA_snn_res.0.25
DimPlot(seuratM, reduction = "umap", pt.size = 0.2, cols = colpal) 
...
```
## plot umaps A4
```{r umap A4, fig.width=29.7, fig.height=21}
Idents(seuratM) <- seuratM$cond
DimPlot(seuratM, reduction = "umap", pt.size = 2, cols = colcond, shuffle = TRUE) + theme(legend.position = "none")

Idents(seuratM) <- seuratM$clusterName
DimPlot(seuratM, reduction = "umap", pt.size = 2, cols = colclusterName)  + theme(legend.position = "none")
```

## plot umaps A4 fb only
```{r umap A4 fb only, fig.width=29.7, fig.height=21}
...
DimPlot(seuratFb, reduction = "umap", pt.size = 2, cols = colcond, shuffle = TRUE) + theme(legend.position = "none")

Idents(seuratFb) <- seuratFb$clusterName
DimPlot(seuratFb, reduction = "umap", pt.size = 2, cols = colclusterName)  + theme(legend.position = "none")
```
